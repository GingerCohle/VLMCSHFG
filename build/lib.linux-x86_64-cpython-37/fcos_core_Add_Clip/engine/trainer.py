...
def foward_detector(cfg, model, images, targets=None, return_maps=True,  DA_ON=True): #target is ground truth
    ...
    model_fcos = model["fcos"]#get this

    if model_fcos.training and DA_ON:
        losses = {}
        images_s, images_t = images
        features_s = model_backbone(images_s.tensors)#get image feature
        features_t = model_backbone(images_t.tensors)
        proposals, proposal_losses, _ ,centerness_s= model_fcos(
            images_s, features_s, targets=targets) #this way

        if with_middle_head:
            score_maps, zeros, _ ,centerness_t= model_fcos(images_t, features_t, targets=None)
            ...
            (features_s, features_t), middle_head_loss = model_middle_head(images, (features_s,features_t), targets=targets, score_maps=score_maps,centerness =(centerness_s,centerness_t))
...
#======================================================================#
    #print(model["middle_head"].graph_generator.class_threshold)
    #model["middle_head"].graph_generator.class_threshold = 0.5
#======================================================================#
    for k in model:
        model[k].train()
    ...
    if not with_DA: # without domain adaptation (we report the results from https://chengchunhsu.github.io/EveryPixelMatters/)
        ...
        for iteration, (images_s,targets_s, _) in enumerate(data_loader_source, start_iter):
            ...
            if iteration % 10000 == 0:
                print('["middle_head"].graph_generator.class_threshold',["middle_head"].graph_generator.class_threshold)
            # if iteration == 3200:
            #     model["middle_head"].graph_generator.class_threshold = 0.2
            # # if iteration == 9900:
            # #     model["middle_head"].graph_generator.class_threshold = 0.25
            # if iteration > 30000 and iteration < 30500 :
            #     model["middle_head"].graph_generator.class_threshold = 0.5
        ...
        for iteration, ((images_s, targets_s, _), (images_t, targets_t, _)) in enumerate(zip(data_loader_source, data_loader_target), start_iter):
            ...
            loss_ccls=loss_dict["cluster_loss"]

            #print("loss_ccls",loss_ccls)
