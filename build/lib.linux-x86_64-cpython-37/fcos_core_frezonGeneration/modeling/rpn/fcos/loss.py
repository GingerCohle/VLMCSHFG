...
class FCOSLossComputation(object):
    ...
    def compute_centerness_targets(self, reg_targets):
        ...
        centerness = (0.6*(top_bottom.min(dim=-1)[0] / top_bottom.max(dim=-1)[0]))* \
                     (0.4*(left_right.min(dim=-1)[0] / left_right.max(dim=-1)[0]))
...
class PrototypeComputation(object):
    ...
    def __call__(self, locations, features, targets, centerness = None):
        ...
        if locations:  # Sampling in the source domain
            ...
            #print("Startiiiiiiiiiiiiiiiiiing" )
            #print("source points 1", len(pos_points))
            ...
            if len(pos_points) > 200:
                ...
                #print("cluster_cen_var1", cluster_cen_var)
                #cluster_cen_var = torch.tensor(cluster_cen_var, requires_grad=True

                #print("cluster_cen_var", cluster_cen_var)
                loss_cluster = torch.cat(cluster_cen_var, dim=0)
                loss_cluster = torch.mean(loss_cluster)
                #print("loss_cluster", loss_cluster)
                #print("loss_cluster",loss_cluster)

                #print("cluster_cen_var2", cluster_cen_var)
                #loss_cluster=torch.mean(cluster_cen_var)
                #print("loss_cluster", loss_cluster)
            ...
            #print("type(neg_points)", type(neg_points))
            try:
                neg_points = neg_points[:int(len(pos_points)/10)]
            except:
                pass
            #print("type(neg_points)", type(neg_points))
            #print("source points 2", len(pos_points))
            ...
            #print("pos_points.size()",pos_points.size())
            return pos_points, pos_labels, pos_labels.new_ones(pos_labels.shape).long(),loss_cluster
            ...
            if len(pos_points)>0:
                ...
                #print("points.size()",pos_points.size())
                #print("endddddddddddddddddddddddddddddddddddddddddddddd")
